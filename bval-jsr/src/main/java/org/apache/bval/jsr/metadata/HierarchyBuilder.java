...
public class HierarchyBuilder extends CompositeBuilder {
    private class HierarchyDelegate implements MetadataBuilder.ForBean {
        private final Metas<Class<?>> hierarchyType;
        private final MetadataBuilder.ForBean delegate;

        HierarchyDelegate(Class<?> hierarchyType, MetadataBuilder.ForBean delegate) {
            super();
        ...
        public MetadataBuilder.ForClass getClass(Metas<Class<?>> meta) {
            return delegate.getClass(hierarchyType);
        }
        ...
        public Map<Signature, MetadataBuilder.ForExecutable<Method>> getMethods(Metas<Class<?>> meta) {
            return delegate.getMethods(hierarchyType);
        }
    }
    ...
    public MetadataBuilder.ForBean forBean(Class<?> beanClass) {
         ...
         */
        delegates.add(Optional.of(beanClass).map(getBeanBuilder).orElse(EmptyBuilder.instance().forBean()));
        ...
        // skip Object.class; skip null/empty hierarchy builders, mapping others to HierarchyDelegate
        hierarchy
            .forEachRemaining(t -> Optional.of(t).filter(Predicate.isEqual(Object.class).negate()).map(getBeanBuilder)
                .filter(b -> !b.isEmpty()).map(b -> new HierarchyDelegate(t, b)).ifPresent(delegates::add));

        // if we have nothing but empty builders (which should only happen for absent custom metadata), return empty:
